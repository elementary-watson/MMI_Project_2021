...
public class Network : MonoBehaviourPunCallbacks
{
    ...
    private string playerColorImgRef;
    ...
    public String getPlayerColor()
    {
        ...
        if (myPlayerColor != null)
            temp = myPlayerColor.Remove(0,5);
        print("Color after remove: " + temp);
        int i = 0;
        foreach(string item in colorFileList)
        {
            if (item.Contains(temp)) 
            {
                return colorFileList[i];
            }
            i++;
        }
        return null;
    ...
    public void giveColor()
    {
        ...
        //einer weniger als zum starten gebraucht wird!
    ...
    public void Awake()
    {
        ...
        RandomColor();
        myRoomOptions = new RoomOptions() { MaxPlayers = 5, IsVisible = true, IsOpen = true };
        roomMaxPlayerRef = 5;
        //canJoin = true;
    }
    ...
    //XOF Funktion der folgenden Methode unbekannt. Steht in Relation mit Lobbyraum verlassen
    //XOF---> public void OnPhotonPlayerDisconnected(){}
